...
/**
 * Saves fitness_certificates and branding_priorities for a train.
 *
 * Callers must pass branding_priorities as an ARRAY ([] for no branding),
 * never undefined. Passing undefined previously caused the field to be silently
 * skipped, meaning bulk-uploaded trains with branding_type=None never had
 * their branding cleared.
 *
 * fitness_certificates: pass null to leave existing value untouched.
 * branding_priorities:  pass [] to explicitly clear branding.
 */
export const saveMasterData = async ({ trainId, fitness_certificates, branding_priorities, userId, userName, userEmail }) => {
    try {
        ...
        // fitness_certificates: write if provided (non-null), otherwise leave existing
        if (fitness_certificates != null) {
            payload.fitness_certificates = { ...fitness_certificates };
        }

        // branding_priorities: write if it's an array (including []).
        // [] explicitly clears old branding — this is intentional.
        if (Array.isArray(branding_priorities)) {
